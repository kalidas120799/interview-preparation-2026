...
Positions → 
   left = 0, right = n-1   (or same direction)
...
console.log(twoSum([2,7,4,15],6)) // [ 1, 3 ]
...
console.log(reversePrefix("abcdefd","d")) //dbcaefd
...
function threeSum(nums) {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        while (left < end) {
            ...
            if (sum === 0) {
                result[[nums[i], nums[left], nums[end]].join("-")] = [nums[i], nums[left], nums[end]]
                left++
                end--
            } else if (sum < 0) { left++; }
            else { end-- }
...
console.log(moveZeroesV1([0,1,0,3,0,12])); // [ 1, 3, 12, 0, 0 ]
